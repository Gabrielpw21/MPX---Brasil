$(function(){
	$(window).scroll(function(){
		...
		// Passa por cada elemento com a classe .paralax
		...
		if ( window_scrolltop >= obj.position().top - obj.height() 
			...
			var divisor = typeof obj.attr('data-divisor') == 'undefined' ? 1000 : obj.attr('data-divisor');
			
			// Corrige a diferença do primeiro elemento
			if ( obj.is(':first-child') ) {
				var bg_pos = ( window_scrolltop - obj.position().top ) / divisor;
			} else {
				var bg_pos = ( window_scrolltop - obj.position().top + ( obj.height() - 200 ) ) / divisor;
			}
			...
			obj.children('.w-50').css({
				'bottom' : ( window_scrolltop - obj.position().top + 450 ) + 'px'
			});
